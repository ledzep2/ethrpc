...
func (s *EthRPCTestSuite) TestTraceTransaction() {
	...
	traces, err := s.rpc.TraceTransaction("0x17104ac9d3312d8c136b7f44d4b8b47852618065ebfa534bd2d3b5ef218ca1f3")
	s.Require().Nil(err)
	s.Require().NotNil(traces)
	s.Require().Equal(3, len(traces))
	s.Require().Equal("call", traces[2].Action.CallType)
	s.Require().Equal("0x1c39ba39e4735cb65978d4db400ddd70a72dc750", traces[2].Action.From)
	s.Require().Equal(hexInt(0x8fc), traces[2].Action.Gas)
	s.Require().Equal("0x", traces[2].Action.Input)
	s.Require().Equal("0x70faa28a6b8d6829a4b1e649d26ec9a2a39ba413", traces[2].Action.To)
	s.Require().Equal(hexBig(newBigInt("8797440000000000000")), traces[2].Action.Value)
	s.Require().Equal("0x7eb25504e4c202cf3d62fd585d3e238f592c780cca82dacb2ed3cb5b38883add", traces[2].BlockHash)
	s.Require().Equal(3068185, traces[2].BlockNumber)
	s.Require().Equal(hexInt(0x0), traces[2].Result.GasUsed)
	s.Require().Equal("0x", traces[2].Result.Output)
	s.Require().Equal(0, traces[2].Subtraces)
	s.Require().Equal(1, len(traces[2].TraceAddress))
	s.Require().Equal(1, traces[2].TraceAddress[0])
	s.Require().Equal("0x17104ac9d3312d8c136b7f44d4b8b47852618065ebfa534bd2d3b5ef218ca1f3", traces[2].TransactionHash)
	s.Require().Equal(2, traces[2].TransactionPosition)
	s.Require().Equal("call", traces[2].Type)
}
